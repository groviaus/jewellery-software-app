...
1. **App ID**: Change `com.yourcompany.jewellery` to your actual app ID (e.g., `com.yourdomain.jewellery`)
2. **Vercel URL**: Change `https://jewellery-software-app.vercel.app` to your actual Vercel deployment URL

Example:
```typescript
appId: 'com.yourdomain.jewellery',
// ...
...
2. **In a new terminal, sync Capacitor for local development:**
   ...
   CAPACITOR_SERVER_URL=http://192.168.1.XXX:3000 pnpm cap:sync
   ```
...
## Step 3: Build and Test in Android Studio
...
- [ ] API calls work (test creating/viewing inventory, customers, etc.)
...
1. **Create a Keystore** (one-time setup):
   ```bash
   keytool -genkey -v -keystore jewellery-release.keystore -alias jewellery -keyalg RSA -keysize 2048 -validity 10000
   ```
...
3. **Update capacitor.config.ts** or set environment variables:
   ```bash
   export ANDROID_KEYSTORE_PATH=/path/to/jewellery-release.keystore
   export ANDROID_KEYSTORE_ALIAS=jewellery
   ```
...
### Issue: "ERR_CLEARTEXT_NOT_PERMITTED" or "Cannot connect to localhost:3000"
**Solution:**
...
- If you want to fix it (optional), it's usually in `build.gradle` files, but not necessary
...
4. **Create release APK** (see Step 4)
...
1. ✅ Update `capacitor.config.ts` with your app ID and Vercel URL
2. ✅ Test locally with dev server
3. ✅ Build and test in Android Studio
4. ✅ Deploy to Vercel
